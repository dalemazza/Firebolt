    ...
    var handler = {
        ...
        options: {
            ...
            cert:{
                value: "",
                required: true,
                description: "The full patch to the SSL cert (PEM)"
            },
            private_key:{
                value: "",
                required: true,
                description: "The full patch to the SSL private key (PEM)"
            },
        },

        description: "HTTPs handler using AES256 encryption",
...
exports.run = async function (app, run) {
    var script = path.join(__dirname,"https_handler.py");

    var command = script +" -p " + run.options.port.value + " -k " + run.options.key.value + " -i " + run._id + " -c " + run.options.cert.value + " --keyfile " + run.options.private_key.value;
    ...
    var child = child_process.execFile("sudo", [python].concat(command.split(" ")),{}, function (error, stdout, stderr) {
